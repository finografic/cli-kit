...
/**
 * Create a fresh confirm-state object. Pass the same instance to every `confirmFileWrite` call in a single
 * command run so yes-to-all is honoured.
 */
...
/**
 * Render a unified diff of `currentContent` → `proposedContent` to stdout. Lines are coloured: `+` green, `-`
 * red, `@@` cyan, `---`/`+++` bold. The unified-diff header lines (Index / ===) are suppressed for a cleaner
 * output.
 */
 ...
 * - Otherwise prompts: yes / skip / yes-to-all. Choosing yes-to-all sets `state.yesAll = true` for subsequent
 *   calls.
 *
